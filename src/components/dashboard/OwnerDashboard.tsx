...
export default function OwnerDashboard() {
  ...
  const stats = [
    { label: "Today's Sales", value: todaySalesTotal !== null ? fmt(todaySalesTotal) : '…', icon: Banknote, color: 'text-green-700 bg-green-100' },
  ...
  const blocks = [
    ...
    { href: '/allotments', label: 'Allotments', icon: ClipboardList, color: 'bg-indigo-600', bg: 'bg-indigo-50', border: 'border-indigo-200' },
    { href: '/calculator', label: t('calculator'), icon: TrendingUp, color: 'bg-teal-600', bg: 'bg-teal-50', border: 'border-teal-200' },
    { href: '/notebook', label: 'Ledger', icon: BookOpen, color: 'bg-orange-600', bg: 'bg-orange-50', border: 'border-orange-200' },
    ...
    { href: '/staff', label: 'Manage Staff', icon: Users, color: 'bg-red-600', bg: 'bg-red-50', border: 'border-red-200' },
  ];
      ...
      {productionAlertsCount !== null && productionAlertsCount > 0 ? (
        <Link href="/reports?tab=production" className="block bg-red-50 rounded-2xl shadow-sm border border-red-200 p-5 active:scale-95 transition-transform">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-red-800 text-lg">Production Alerts</h2>
              <p className="text-sm font-semibold text-red-600 mt-0.5">{productionAlertsCount} plants need immediate propagation.</p>
            </div>
          </div>
        </Link>
      ) : (
